...
3. Предложи расчёт
4. Отправь каталог [SEND_CATALOG]
...
[STAGE:lose] — отказ
[SEND_CATALOG] — отправить каталог нужного продукта
...
app.post("/webhook", async (req, res) => {
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      // Отправляем каталог
      if (sendCatalog) {
        const histText = (histories[chatId] || [])
        ...
        await wazzupSendDocument(chatId, phone, CATALOGS[product][lang],
          `Каталог ${isQuartz ? "Mr. Quartz" : "Plastogum"} 📋`);
      }
      if (sendColorCat) {
