...
async function wazzupSendDocument(chatId, phone, filePath, caption) {
  try {
    ...
    console.log("Sending doc base64:", filename, "channel:", ch, "to:", phone);
...
app.post("/webhook", async (req, res) => {
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      // ✅ ИСПРАВЛЕНО: передаём chatId вместо null
      ...
      if (sendChecklist) {
        ...
        // ✅ ИСПРАВЛЕНО: передаём chatId вместо null
        await wazzupSendMessage(chatId, phone, checklist);
      }
      ...
      // ✅ ИСПРАВЛЕНО: передаём chatId вместо null
      await wazzupSendMessage(chatId, phone, "Произошла ошибка, попробуйте ещё раз.");
    }
  }
});
