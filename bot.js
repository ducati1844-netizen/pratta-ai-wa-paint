...
const recentBotSends   = {}; // phone -> timestamp of last bot send (to filter own echoes)
...
async function wazzupSendMessage(chatId, phone, text) {
  try {
    ...
    else { recentBotSends[phone] = Date.now(); console.log("Wazzup message sent to", phone); }
  } catch(e) {
...
app.post("/webhook", async (req, res) => {
  ...
  for (const msg of messages) {
    // FIX: filter echo messages — ignore bot's own echoes, detect real manager writes
    if (msg.isEcho === true) {
      const ePhone = msg.chatId || msg.phone || "";
      if (ePhone) {
        const lastSend = recentBotSends[ePhone] || 0;
        const isBotEcho = (Date.now() - lastSend) < 60000; // bot sent within last 60s
        if (!isBotEcho) {
