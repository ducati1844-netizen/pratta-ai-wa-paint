  ...
  const makeFollowUp = (attempt) => async () => {
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-haiku-4-5-20251001", max_tokens: 200, system: getSystem(), messages: msgs })
      });
      const data = await res.json();
...
async function processBuffer(chatId) {
  ...
  if (combinedText.length > 3) clientLang[chatId] = detectLang(combinedText);
  ...
  try {
    const reply = await askClaude(chatId, combinedText);
...
app.post("/webhook", async (req, res) => {
  ...
  for (const msg of messages) {
    ...
    // ── Менеджер пишет вручную — глушим бота на 30 мин ──────────────────────
    if (msg.isEcho === true) {
      if (botActive[chatId] || dealIds[chatId]) {
        ...
        console.log(`WA bot PAUSED for ${phone} (manager writing)`);
      }
      continue;
    }
