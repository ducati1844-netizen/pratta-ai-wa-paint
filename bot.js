...
async function wazzupSendDocument(phone, filePath, caption) {
  try {
    ...
    form.append("channelId", WAZZUP_CHANNEL);
...
function scheduleNoAnswerSequence(chatId, phone) {
  ...
  const t1 = setTimeout(async () => {
    ...
    await wazzupSendMessage(null, phone, "Привет! 👋 Вы недавно интересовались нашей краской.\n\nЕщё актуально? Подберу вариант под вашу задачу.");
    console.log("WA no-answer 1st silence:", phone);
  }, 1 * 60 * 60 * 1000);
  ...
  const t2 = setTimeout(async () => {
    ...
    await wazzupSendMessage(null, phone, "Добрый день 🙂\n\nЕсли вопрос по краске ещё актуален — я здесь. Могу рассчитать количество и стоимость.");
    console.log("WA no-answer 2nd silence:", phone);
  }, 24 * 60 * 60 * 1000);
  ...
  const t3 = setTimeout(async () => {
    ...
    await wazzupSendMessage(null, phone, "Последнее сообщение 🙏\n\nКогда будете готовы — напишите. Всегда поможем!");
    setTimeout(async () => {
...
function setFollowUpTimer(chatId, phone) {
  ...
  const makeFollowUp = (attempt) => async () => {
    ...
    const stage = currentStage[chatId] || "answer";
    const hist = histories[chatId] || [];
    const prompt = `Клиент замолчал в WhatsApp. Попытка дожима №${attempt}. Этап: "${stage}".
Напиши короткое (1-2 предложения) живое сообщение для WhatsApp.
Только текст, без тегов.`;
    try {
      ...
      if (text) await wazzupSendMessage(null, phone, text);
    } catch(e) { console.error("WA follow-up error:", e.message); }
  };

  const t1 = setTimeout(makeFollowUp(1), 1 * 60 * 60 * 1000);
  const t2 = setTimeout(makeFollowUp(2), 24 * 60 * 60 * 1000);
  const t3 = setTimeout(async () => {
...
app.post("/webhook", async (req, res) => {
  ...
  for (const msg of messages) {
    // Только входящие от клиента (не исходящие)
